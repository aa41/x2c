...
final class LayoutEmitter {
    ...
    String emit(LayoutNode node, LayoutNode parentNode, String parentVariable) {
        ...
        String params = null;
        if (parentNode == null) {
            out.line(variable + ".setLayoutParams(new ViewGroup.LayoutParams("
    ...
    private String emitLayoutParams(LayoutNode node, LayoutNode parent, String variable) {
            ...
            switch (kind) {
                ...
                case TABLE_ROW:
                    ...
                    if (node.attributes.containsKey("layout_column")) {
                        out.line(params + ".column = " + integerExpression(node.attributes.get("layout_column")) + ";");
                    }
                    if (node.attributes.containsKey("layout_span")) {
                        out.line(params + ".span = " + integerExpression(node.attributes.get("layout_span")) + ";");
                    }
                    break;
                case RADIO:
                    ...
                    break;
                case FRAME:
                    out.line("FrameLayout.LayoutParams " + params + " = new FrameLayout.LayoutParams("
                            + width + ", " + height + ");");
                    emitMargins(node, params);
                    if (node.attributes.containsKey("layout_gravity")) {
                        out.line(params + ".gravity = " + gravityExpression(node.attributes.get("layout_gravity")) + ";");
                    }
                    break;
                case RELATIVE:
                    out.line("RelativeLayout.LayoutParams " + params + " = new RelativeLayout.LayoutParams("
                            + width + ", " + height + ");");
                    emitMargins(node, params);
                ...
                case GRID:
                    ...
                    break;
                case ABSOLUTE:
                    out.line("AbsoluteLayout.LayoutParams " + params
                    ...
                    break;
                case TOOLBAR:
                    out.line("Toolbar.LayoutParams " + params + " = new Toolbar.LayoutParams("
                            + width + ", " + height + ");");
                    emitMargins(node, params);
                    if (node.attributes.containsKey("layout_gravity")) {
                        out.line(params + ".gravity = "
                                + gravityExpression(node.attributes.get("layout_gravity")) + ";");
                    }
                    break;
                case ACTION_MENU:
                    out.line("ActionMenuView.LayoutParams " + params + " = new ActionMenuView.LayoutParams("
                            + width + ", " + height + ");");
                    emitMargins(node, params);
                    if (node.attributes.containsKey("layout_gravity")) {
                        out.line(params + ".gravity = "
        ...
        if (container.layoutParamsFactoryClass() == null) {
            out.line(container.layoutParamsClass() + " " + params + " = new "
                    + container.layoutParamsClass() + "(" + width + ", " + height + ");");
        } else {
            out.line(container.layoutParamsClass() + " " + params + " = "
                    + container.layoutParamsFactoryClass() + "."
                    + container.layoutParamsFactoryMethod() + "(context, " + width + ", " + height + ");");
        }
    ...
    private void emitLinearLayoutParams(
        ...
        String weight = node.attributes.containsKey("layout_weight")
                ? floatExpression(node.attributes.get("layout_weight")) : "0.0f";
        out.line(type + " " + params + " = new " + type + "(" + width + ", " + height + ", " + weight + ");");
        emitMargins(node, params);
        if (node.attributes.containsKey("layout_gravity")) {
            out.line(params + ".gravity = " + gravityExpression(node.attributes.get("layout_gravity")) + ";");
        }
    ...
    private void emitRelativeLayoutRules(LayoutNode node, String params) {
        Map<String, String> anchoredRules = mapOfEntries(
                entry("layout_above", "ABOVE"), entry("layout_below", "BELOW"),
                entry("layout_toLeftOf", "LEFT_OF"), entry("layout_toRightOf", "RIGHT_OF"),
                entry("layout_toStartOf", "START_OF"), entry("layout_toEndOf", "END_OF"),
                entry("layout_alignLeft", "ALIGN_LEFT"), entry("layout_alignTop", "ALIGN_TOP"),
                entry("layout_alignRight", "ALIGN_RIGHT"), entry("layout_alignBottom", "ALIGN_BOTTOM"),
                entry("layout_alignStart", "ALIGN_START"), entry("layout_alignEnd", "ALIGN_END"),
        ...
        Map<String, String> booleanRules = mapOfEntries(
        ...
        if (node.attributes.containsKey("layout_alignWithParentIfMissing")) {
            out.line(params + ".alignWithParent = "
    ...
    private void emitGridLayoutParams(LayoutNode node, String params, String width, String height) {
        ...
        if (node.attributes.containsKey("layout_gravity")) {
            out.line(params + ".setGravity(" + gravityExpression(node.attributes.get("layout_gravity")) + ");");
        }
    }
    ...
    private void emitGridSpec(LayoutNode node, String params, String axis) {
        ...
        String start = node.attributes.containsKey(startName)
                ? integerExpression(node.attributes.get(startName)) : "GridLayout.UNDEFINED";
        String span = node.attributes.containsKey(spanName)
                ? integerExpression(node.attributes.get(spanName)) : "1";
    ...
    private void applyProperties(LayoutNode node, String variable) {
        ...
        if (TEXT_VIEW_TAGS.contains(node.tag)) {
            ...
            if (attrs.containsKey("textColor")) {
                out.line(variable + ".setTextColor(" + colorStateListExpression(attrs.get("textColor")) + ");");
            }
            if (attrs.containsKey("textSize")) {
                String value = attrs.get("textSize");
                if (value.startsWith("@dimen/")) {
                    out.line(variable + ".setTextSize(TypedValue.COMPLEX_UNIT_PX, " + dimenReference(value) + ");");
                } else {
                    Dimension dimen = parseDimension(null, value);
                    out.line(variable + ".setTextSize(" + typedValueUnit(dimen.unit) + ", " + floatLiteral(dimen.value) + ");");
                }
            }
            ...
            if (attrs.containsKey("scaleType")) {
                out.line(variable + ".setScaleType(ImageView.ScaleType." + scaleTypeConstant(attrs.get("scaleType")) + ");");
            }
            if (attrs.containsKey("adjustViewBounds")) {
                out.line(variable + ".setAdjustViewBounds(" + booleanExpression(attrs.get("adjustViewBounds")) + ");");
            }
            emitBooleanSetter(attrs, variable, "cropToPadding", "setCropToPadding");
            if (attrs.containsKey("imageTint")) {
                out.line(variable + ".setImageTintList(" + colorStateListExpression(attrs.get("imageTint")) + ");");
            }
    ...
    private void emitFrameworkContainerProperties(LayoutNode node, String variable) {
        ...
        if (setOf("LinearLayout", "TableRow", "RadioGroup").contains(node.tag)) {
            out.line(variable + ".setOrientation(LinearLayout."
            ...
            if (attrs.containsKey("baselineAlignedChildIndex")) {
                out.line(variable + ".setBaselineAlignedChildIndex("
                        + integerExpression(attrs.get("baselineAlignedChildIndex")) + ");");
            }
            emitBooleanSetter(attrs, variable, "measureWithLargestChild", "setMeasureWithLargestChildEnabled");
        } else if (node.tag.equals("TableLayout")) {
            if (attrs.containsKey("weightSum")) {
                out.line(variable + ".setWeightSum(" + floatExpression(attrs.get("weightSum")) + ");");
            }
            emitTableColumns(attrs, variable, "stretchColumns", "setColumnStretchable", "setStretchAllColumns");
            emitTableColumns(attrs, variable, "shrinkColumns", "setColumnShrinkable", "setShrinkAllColumns");
            ...
            if (attrs.containsKey("foregroundGravity")) {
                out.line(variable + ".setForegroundGravity("
            ...
            out.line(variable + ".setOrientation(GridLayout."
                    + (attrs.getOrDefault("orientation", "horizontal").equals("vertical")
                    ? "VERTICAL" : "HORIZONTAL") + ");");
            ...
            if (attrs.containsKey("columnCount")) {
                out.line(variable + ".setColumnCount(" + integerExpression(attrs.get("columnCount")) + ");");
            }
            ...
            if (attrs.containsKey("alignmentMode")) {
                out.line(variable + ".setAlignmentMode(GridLayout."
                        + (attrs.get("alignmentMode").equals("alignBounds") ? "ALIGN_BOUNDS" : "ALIGN_MARGINS")
                        + ");");
            }
        ...
        if (FRAMEWORK_CONTAINERS.contains(node.tag)) {
            ...
            emitBooleanSetter(attrs, variable, "motionEventSplittingEnabled", "setMotionEventSplittingEnabled");
        }
    }

    private void emitTableColumns(
            Map<String, String> attrs, String variable, String attribute, String setter, String allSetter) {
    ...
    private void emitTextProperties(LayoutNode node, String variable) {
        ...
        if (attrs.containsKey("textStyle")) {
            out.line(variable + ".setTypeface(" + variable + ".getTypeface(), "
        ...
        if (attrs.containsKey("hintTextColor")) {
            out.line(variable + ".setHintTextColor(" + colorStateListExpression(attrs.get("hintTextColor")) + ");");
        }
        ...
        if (attrs.containsKey("ellipsize")) {
            ...
            out.line(variable + ".setEllipsize(" + (value.equals("none") ? "null"
        ...
        if (attrs.containsKey("lineSpacingExtra") || attrs.containsKey("lineSpacingMultiplier")) {
            out.line(variable + ".setLineSpacing("
        ...
        if (attrs.containsKey("inputType")) {
            out.line(variable + ".setInputType(" + integerFlagsExpression(attrs.get("inputType"), true) + ");");
        }
        if (attrs.containsKey("imeOptions")) {
            out.line(variable + ".setImeOptions(" + integerFlagsExpression(attrs.get("imeOptions"), false) + ");");
        }
        ...
        if (COMPOUND_BUTTON_TAGS.contains(node.tag) && attrs.containsKey("buttonTint")) {
            out.line(variable + ".setButtonTintList(" + colorStateListExpression(attrs.get("buttonTint")) + ");");
        }
    }

    private void emitProgressProperties(LayoutNode node, String variable) {
        Map<String, String> attrs = node.attributes;
        if (attrs.containsKey("max")) out.line(variable + ".setMax(" + integerExpression(attrs.get("max")) + ");");
        if (attrs.containsKey("progress")) out.line(variable + ".setProgress(" + integerExpression(attrs.get("progress")) + ");");
        if (attrs.containsKey("secondaryProgress")) out.line(variable + ".setSecondaryProgress("
        ...
        if (attrs.containsKey("progressTint")) {
            out.line(variable + ".setProgressTintList(" + colorStateListExpression(attrs.get("progressTint")) + ");");
        }
        if (node.tag.equals("SeekBar") && attrs.containsKey("thumb")) {
            out.line(variable + ".setThumb(X2cDrawables."
                    + javaName(referenceName(attrs.get("thumb"), "drawable")) + "(context));");
        }
        if (node.tag.equals("RatingBar")) {
            if (attrs.containsKey("rating")) out.line(variable + ".setRating(" + floatExpression(attrs.get("rating")) + ");");
            if (attrs.containsKey("numStars")) out.line(variable + ".setNumStars(" + integerExpression(attrs.get("numStars")) + ");");
            if (attrs.containsKey("stepSize")) out.line(variable + ".setStepSize(" + floatExpression(attrs.get("stepSize")) + ");");
    ...
    private void emitCommon(Map<String, String> attrs, String variable) {
        if (attrs.containsKey("background")) {
            ...
            if (value.startsWith("@color/") || value.startsWith("#")) {
                ...
                out.line(variable + ".setBackground(X2cDrawables." + javaName(referenceName(value, "drawable")) + "(context));");
            }
        }
        if (attrs.containsKey("backgroundTint")) {
            out.line(variable + ".setBackgroundTintList("
                    + colorStateListExpression(attrs.get("backgroundTint")) + ");");
        }
        if (attrs.containsKey("foreground")) {
            out.line("if (Build.VERSION.SDK_INT >= 23) " + variable + ".setForeground("
                    + foregroundExpression(attrs.get("foreground")) + ");");
        }
        if (attrs.containsKey("visibility")) {
            out.line(variable + ".setVisibility(View." + attrs.get("visibility").toUpperCase(Locale.ROOT) + ");");
        }
        ...
        if (attrs.containsKey("contentDescription")) {
            out.line(variable + ".setContentDescription(" + stringExpression(attrs.get("contentDescription")) + ");");
        }
        if (attrs.containsKey("tag")) {
            out.line(variable + ".setTag(" + stringExpression(attrs.get("tag")) + ");");
        }
        if (attrs.containsKey("tooltipText")) {
            out.line("if (Build.VERSION.SDK_INT >= 26) " + variable + ".setTooltipText("
                    + stringExpression(attrs.get("tooltipText")) + ");");
        }
        if (attrs.containsKey("transitionName")) {
            out.line(variable + ".setTransitionName(" + stringExpression(attrs.get("transitionName")) + ");");
        }
    ...
    private void emitPadding(Map<String, String> attrs, String variable) {
        ...
        if (attrs.containsKey("paddingStart") || attrs.containsKey("paddingEnd")) {
            ...
            out.line(variable + ".setPaddingRelative(" + roundedDimension(start) + ", " + roundedDimension(top) + ", "
                    + roundedDimension(end) + ", " + roundedDimension(bottom) + ");");
            return;
        }
        ...
        if (all != null || attrs.keySet().stream().anyMatch(key -> key.startsWith("padding"))) {
            out.line(variable + ".setPadding(" + roundedDimension(left) + ", " + roundedDimension(top) + ", "
                    + roundedDimension(right) + ", " + roundedDimension(bottom) + ");");
        }
    }
    ...
    private void emitMargins(LayoutNode node, String params) {
        ...
        if (all != null || attrs.keySet().stream().anyMatch(key -> key.startsWith("layout_margin"))) {
            out.line(params + ".setMargins(" + roundedDimension(left) + ", " + roundedDimension(top) + ", "
                    + roundedDimension(right) + ", " + roundedDimension(bottom) + ");");
        }
        if (attrs.containsKey("layout_marginStart")) {
            out.line(params + ".setMarginStart(" + roundedDimension(attrs.get("layout_marginStart")) + ");");
        }
    ...
    private String constructorExpression(LayoutNode node) {
        ...
        if ("CONTEXT_ATTRS_DEF_STYLE".equals(constructor)) return "new " + node.javaType + "(context, null, 0)";
    ...
    private void emitBooleanSetter(Map<String, String> attrs, String variable, String attribute, String setter) {
        if (attrs.containsKey(attribute)) {
            out.line(variable + "." + setter + "(" + booleanExpression(attrs.get(attribute)) + ");");
        }
    }

    private void emitFloatSetter(Map<String, String> attrs, String variable, String attribute, String setter) {
        if (attrs.containsKey(attribute)) {
            out.line(variable + "." + setter + "(" + floatLiteral(floatValue(attrs.get(attribute))) + ");");
        }
    }

    private void emitDimensionSetter(Map<String, String> attrs, String variable, String attribute, String setter) {
        if (attrs.containsKey(attribute)) {
            out.line(variable + "." + setter + "(" + dimensionValueExpression(attrs.get(attribute)) + ");");
        }
    }
    ...
    private String dimensionValueExpression(String value) {
        ...
        return "TypedValue.applyDimension(" + typedValueUnit(dimen.unit) + ", " + floatLiteral(dimen.value)
    ...
    private String stringExpression(String value) {
        if (value.startsWith("@string/")) {
            return "X2cValues.Strings." + javaName(referenceName(value, "string"));
        }
    ...
    private String colorValueExpression(String value) {
        if (value.startsWith("@color/")) {
            return "X2cValues.Colors." + javaName(referenceName(value, "color"));
        }
    ...
    private String foregroundExpression(String value) {
        if (value.startsWith("@drawable/")) {
            return "X2cDrawables." + javaName(referenceName(value, "drawable")) + "(context)";
        }
    ...
    private String booleanExpression(String value) {
        if (value.startsWith("@bool/")) {
            return "X2cValues.Bools." + javaName(referenceName(value, "bool"));
        }
        return value;
    }
    ...
    private String integerExpression(String value) {
        if (value.startsWith("@integer/")) {
            return "X2cValues.Integers." + javaName(referenceName(value, "integer"));
        }
        return value;
    }
    ...
    private String inputTypeConstant(String value) {
        ...
        if ("textUri".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_URI";
        if ("textEmailAddress".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS";
        if ("textEmailSubject".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_EMAIL_SUBJECT";
        if ("textShortMessage".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_SHORT_MESSAGE";
        if ("textLongMessage".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_LONG_MESSAGE";
        if ("textPersonName".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PERSON_NAME";
        if ("textPostalAddress".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_POSTAL_ADDRESS";
        if ("textPassword".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD";
        if ("textVisiblePassword".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD";
        if ("textWebEditText".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_WEB_EDIT_TEXT";
        if ("textFilter".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_FILTER";
        if ("textPhonetic".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PHONETIC";
        if ("textWebEmailAddress".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_WEB_EMAIL_ADDRESS";
        if ("textWebPassword".equals(value)) return "InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_WEB_PASSWORD";
        ...
        if ("numberPassword".equals(value)) return "InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_VARIATION_PASSWORD";
        if ("phone".equals(value)) return "InputType.TYPE_CLASS_PHONE";
        if ("datetime".equals(value)) return "InputType.TYPE_CLASS_DATETIME";
        if ("date".equals(value)) return "InputType.TYPE_CLASS_DATETIME | InputType.TYPE_DATETIME_VARIATION_DATE";
        if ("time".equals(value)) return "InputType.TYPE_CLASS_DATETIME | InputType.TYPE_DATETIME_VARIATION_TIME";
    ...
    private String dimenReference(String value) {
        return "X2cValues.Dimens." + javaName(referenceName(value, "dimen")) + "(context)";
    }
