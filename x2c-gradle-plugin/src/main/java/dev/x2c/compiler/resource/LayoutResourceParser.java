...
final class LayoutResourceParser {
    ...
    static void parseLayout(ResourceFile source, Model model) {
        ...
        if (!isContainer(root)) {
            throw fail(source.file, "Layout root must be a supported framework ViewGroup or registered custom ViewGroup");
        }
        for (String attribute : root.attributes.keySet()) {
            if (attribute.startsWith("layout_")
                ...
                throw fail(source.file, "Root layout cannot use parent LayoutParams attribute: android:" + attribute);
            }
        }
    ...
    static LayoutNode parseLayoutNode(File file, Element element, Model model, LayoutNode parent) {
        ...
        CustomViewRegistry.ViewSpec customView = FRAMEWORK_VIEW_TAGS.contains(resolvedTag)
                ? null
                : model.customViews.find(resolvedTag);
        if (!FRAMEWORK_VIEW_TAGS.contains(resolvedTag) && customView == null) {
            throw fail(file, "Unregistered custom View: " + resolvedTag + ". Declare it in x2c.customViewsFile");
        }
        ...
        for (int index = 0; index < attributes.getLength(); index++) {
            ...
            CustomViewRegistry.AttributeSpec parentLayoutAttribute = name == null || parent == null || parent.container() == null
                    ? null
            ...
            CustomViewRegistry.AttributeSpec customAttribute = customView == null ? null : customView.attribute(name);
    ...
    static void validateLayoutAttributes(File file, LayoutNode node, Model model) {
        for (Map.Entry<String, String> item : node.attributes.entrySet()) {
            ...
            if ("id".equals(name)) {
                ...
                if (id.kind == IdKind.LOCAL && !model.declaredIds.contains(id.name)) {
                    throw fail(file, "Unknown ID reference: " + value
                            + ". Declare it with @+id/" + id.name + " or <item type=\"id\" name=\""
                            + id.name + "\" />");
                }
            ...
            } else if (setOf("layout_margin", "layout_marginLeft", "layout_marginTop", "layout_marginRight",
                    "layout_marginBottom", "layout_marginStart", "layout_marginEnd", "padding", "paddingLeft",
                    "paddingTop", "paddingRight", "paddingBottom", "paddingStart", "paddingEnd", "textSize",
                    "elevation", "translationX", "translationY", "translationZ", "minWidth", "minHeight",
                    "lineSpacingExtra", "layout_x", "layout_y").contains(name)) {
                resolveDimension(file, value, model);
            } else if ("background".equals(name)) {
                resolveColorOrDrawable(file, value, model, false);
            } else if (setOf("textColor", "backgroundTint", "hintTextColor", "buttonTint", "imageTint",
                    "progressTint").contains(name)) {
            ...
            } else if (setOf("enabled", "clickable", "longClickable", "focusable", "focusableInTouchMode",
                    "selected", "activated", "saveEnabled", "keepScreenOn", "fitsSystemWindows",
                    "adjustViewBounds", "soundEffectsEnabled", "hapticFeedbackEnabled", "duplicateParentState",
                    "filterTouchesWhenObscured", "isScrollContainer", "clipToOutline", "accessibilityHeading",
                    "screenReaderFocusable", "singleLine", "includeFontPadding", "textAllCaps", "textIsSelectable",
                    "selectAllOnFocus", "checked", "cropToPadding", "indeterminate", "isIndicator", "clipChildren", "clipToPadding",
                    "motionEventSplittingEnabled", "baselineAligned", "measureWithLargestChild", "measureAllChildren",
                    "useDefaultMargins", "rowOrderPreserved", "columnOrderPreserved", "fillViewport",
                    "smoothScrollingEnabled", "layout_alignParentLeft", "layout_alignParentTop",
                    "layout_alignParentRight", "layout_alignParentBottom", "layout_alignParentStart",
                    "layout_alignParentEnd", "layout_centerHorizontal", "layout_centerVertical",
                    "layout_centerInParent", "layout_alignWithParentIfMissing").contains(name)) {
                resolveBoolean(file, value, model, name);
            } else if (setOf("alpha", "rotation", "rotationX", "rotationY", "scaleX", "scaleY", "letterSpacing",
                    "lineSpacingMultiplier", "weightSum", "rating", "stepSize", "layout_rowWeight",
                    "layout_columnWeight").contains(name)) {
                ...
                requireOneOf(file, name, value, "inherit", "gravity", "textStart", "textEnd", "center", "viewStart", "viewEnd");
            ...
            } else if (setOf("maxLines", "minLines", "lines", "baselineAlignedChildIndex", "rowCount",
                    "columnCount", "max", "progress", "secondaryProgress", "numStars", "layout_row",
                    "layout_rowSpan", "layout_column", "layout_columnSpan", "layout_span").contains(name)) {
                resolveInteger(file, value, model, name);
            } else if ("scaleType".equals(name)) {
                requireOneOf(file, name, value, "center", "centerCrop", "centerInside", "fitCenter", "fitStart",
                        "fitEnd", "fitXY", "matrix");
            ...
            } else if (setOf("layout_above", "layout_below", "layout_toLeftOf", "layout_toRightOf",
                    "layout_toStartOf", "layout_toEndOf", "layout_alignLeft", "layout_alignTop",
                    "layout_alignRight", "layout_alignBottom", "layout_alignStart", "layout_alignEnd",
                    "layout_alignBaseline").contains(name)) {
                validateIdReference(file, value, model);
            }
        }
        boolean absolutePadding = node.attributes.keySet().stream()
                .anyMatch(name -> setOf("paddingLeft", "paddingRight").contains(name));
        boolean relativePadding = node.attributes.keySet().stream()
        ...
        boolean absoluteMargins = node.attributes.keySet().stream()
                .anyMatch(name -> setOf("layout_marginLeft", "layout_marginRight").contains(name));
        boolean relativeMargins = node.attributes.keySet().stream()
    ...
    static void validateCustomAttribute(File file, CustomAttributeValue value, Model model) {
        switch (value.spec.type()) {
            case "STRING": resolveString(file, value.value, model); break;
            case "COLOR": resolveColor(file, value.value, model); break;
            case "DIMENSION":
            case "DIMENSION_INT": resolveDimension(file, value.value, model); break;
            case "BOOLEAN": resolveBoolean(file, value.value, model, value.spec.name()); break;
            case "INTEGER": resolveInteger(file, value.value, model, value.spec.name()); break;
            case "FLOAT": parseFloat(file, value.value, value.spec.name()); break;
            case "GRAVITY": parseGravity(file, value.value); break;
            case "DRAWABLE": resolveGeneratedDrawable(file, value.value, model); break;
            case "IMAGE_ASSET": resolveBitmapDrawable(file, value.value, model); break;
            default: throw new AssertionError(value.spec.type());
        }
    }
}
